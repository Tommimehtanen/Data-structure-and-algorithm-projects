...
public:
    ...
    // Estimate of performance: O(n) theta(1)
    // Short rationale for estimate:Find komento on keskimäärin vakioaikainen.
    std::vector<std::pair<WayID, Coord>> ways_from(Coord xy);

    // Estimate of performance: log(n)
    // Short rationale for estimate: find-method perustuu punamusta-puuhun joten sen asymptoottinen tehokkuus on logaritmista.
    std::vector<Coord> get_way_coords(WayID id);

    // Estimate of performance: O(n)
    // Short rationale for estimate:
    // Tyhjentäminen on lineaarinen operaatio
    void clear_ways();

    // Estimate of performance: O(n*m), Theta (nlogm), n: pysähdysten määrä, m: naapuripysähdysten määrä
    // Short rationale for estimate:
    // Ensin etsitään ensimmäisen risteyksen naapuriristeykset.
    // Sen jälkeen etäisyydet niihin lasketaan ja ne lisätään jonoon ja sama prosessi toistetaan.
    // jos kohderisteys löytyy, kauempana olevat pysäkit hylätään
    // lopuksi lasketaan kokonaisväli ja pysähdykset lähtövektoriin.
    ...
    // Estimate of performance: Täysin sama kuin route_any funktiolla
    // Short rationale for estimate:
    ...
    // Short rationale for estimate:
    Distance trim_ways();
    void bf_traversal(Coord b);
    double distance_to(Coord other);
